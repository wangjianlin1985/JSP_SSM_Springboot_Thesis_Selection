...
public class OpusController extends BaseController {
	...
	public void userAdd(Opus opus, BindingResult br,
		...
		opus.setUserObj(userObj);
		
		
		if(opusService.queryOpus(opus.getSubjectObj(), userObj).size() > 0) {
			message = "不要重复提交此论文题目成果！";
			writeJsonResponse(response, success, message);
			return ;
		}
	...
	public void teacherlist(@ModelAttribute("userObj") UserInfo userObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response,HttpSession session) throws Exception {
		if (page==null || page == 0) page = 1;
		if(rows != 0)opusService.setRows(rows);
		String teacherNo = session.getAttribute("teacherNo").toString();
		List<Opus> opusList = opusService.teacherQueryOpus(teacherNo, userObj, page);
	    /*计算总的页数和总的记录数*/
	    opusService.teacherQueryTotalPageAndRecordNumber(teacherNo, userObj);
	    /*获取到总的页码数目*/
	    int totalPage = opusService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = opusService.getRecordNumber();
	...
	public String userFrontlist(@ModelAttribute("subjectObj") Subject subjectObj,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Opus/opus_userFrontquery_result"; 
	}
