...
public class SubSelectController extends BaseController {
	...
	public void userAdd( SubSelect subSelect, BindingResult br,
		...
		int subjectId = subSelect.getSubjectObj().getSubjectId();
		Subject subject = subjectService.getSubject(subjectId);
		if(subSelectService.querySubSelect(subject, userObj, "", "").size() > 0) {
			message = "你已经选过这个题目了！";
			writeJsonResponse(response, success, message);
			return ;
		}
		...
		subSelect.setSelectTime(sdf.format(new java.util.Date()));
		
		subSelect.setShenHeState("待审核");
		subSelect.setUserObj(userObj);
	...
	/*老师界面 ajax方式按照查询条件分页查询学生选题信息*/
	@RequestMapping(value = { "/teacherlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void teacherlist(@ModelAttribute("userObj") UserInfo userObj,String selectTime,String shenHeState,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response,HttpSession session) throws Exception {
		if (page==null || page == 0) page = 1;
		if (selectTime == null) selectTime = "";
		if (shenHeState == null) shenHeState = "";
		String teacherNo = session.getAttribute("teacherNo").toString();
		if(rows != 0)subSelectService.setRows(rows);
		List<SubSelect> subSelectList = subSelectService.teacherQuerySubSelect(teacherNo, userObj, selectTime, shenHeState, page);
	    /*计算总的页数和总的记录数*/
	    subSelectService.teacherQueryTotalPageAndRecordNumber(teacherNo, userObj, selectTime, shenHeState);
	    /*获取到总的页码数目*/
	    int totalPage = subSelectService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = subSelectService.getRecordNumber();
	...
	public String userFrontlist(@ModelAttribute("subjectObj") Subject subjectObj,@ModelAttribute("userObj") UserInfo userObj,String selectTime,String shenHeState,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (selectTime == null) selectTime = "";
		if (shenHeState == null) shenHeState = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "SubSelect/subSelect_userFrontquery_result"; 
	}
