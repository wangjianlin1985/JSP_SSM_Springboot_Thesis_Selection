...
public class SubjectController extends BaseController {
	...
	public void teacherAdd(Subject subject, BindingResult br,
		...
		teacherObj.setTeacherNo(teacherNo);
		subject.setTeacherObj(teacherObj);
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		subject.setAddTime(sdf.format(new java.util.Date()));
	...
	public void userListAll(HttpServletResponse response,HttpSession session) throws Exception {
		...
		ArrayList<SubSelect> subSelectList = subSelectService.querySubSelect(null, userObj, "", "审核通过");
		
		for(SubSelect subSelect:subSelectList) {
			JSONObject jsonSubject = new JSONObject();
			jsonSubject.accumulate("subjectId", subSelect.getSubjectObj().getSubjectId());
			jsonSubject.accumulate("subjectName", subSelect.getSubjectObj().getSubjectName());
			jsonArray.put(jsonSubject);
		}
		out.println(jsonArray.toString());
		out.flush();
