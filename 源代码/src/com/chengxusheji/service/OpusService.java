...
public class OpusService {
    ...
    /*老师 按照查询条件分页查询学生成果记录*/
    public ArrayList<Opus> teacherQueryOpus(String teacherNo,UserInfo userObj,int currentPage) throws Exception { 
     	String where = "where 1=1";
    ...
    /*老师在 当前查询条件下计算总的页数和记录数*/
    public void teacherQueryTotalPageAndRecordNumber(String teacherNo,UserInfo userObj) throws Exception {
     	String where = "where 1=1";
    	if(null != teacherNo)  where += " and t_teacher.teacherNo='" + teacherNo + "'";
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_opus.userObj='" + userObj.getUser_name() + "'";
        recordNumber = opusMapper.queryOpusCount(where);
